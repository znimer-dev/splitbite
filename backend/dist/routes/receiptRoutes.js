...
const splitCalculationService_1 = require("../services/splitCalculationService");
...
const upload = (0, multer_1.default)({
    ...
    limits: {
        fileSize: aws_1.S3_CONFIG.maxFileSize
    },
    fileFilter: (req, file, cb) => {
        if (aws_1.S3_CONFIG.allowedTypes.includes(file.mimetype)) {
            cb(null, true);
        }
        else {
 ...
 */
router.post('/upload', authenticateUser, upload.single('receipt'), async (req, res) => {
    try {
        if (!req.file) {
        ...
        const s3Result = await s3Service_1.S3Service.uploadFile(req.file.buffer, req.file.originalname, req.file.mimetype);
        // Step 2: Process with Textract
        console.log('🔍 Processing image with Textract...');
        const extractedData = await textractService_1.TextractService.extractReceiptData(s3Result.bucket, s3Result.key);
        ...
        try {
            let restaurant = await Restaurant_1.default.findOne({
                userId: new mongoose_1.default.Types.ObjectId(userId),
            ...
            if (restaurant) {
                ...
                restaurant = new Restaurant_1.default({
                    name: receipt.restaurantName,
                    visitCount: 1,
...
router.get('/', authenticateUser, async (req, res) => {
    try {
        ...
        const receipts = await Receipt_1.default.find({ userId: new mongoose_1.default.Types.ObjectId(userId) })
            .sort({ createdAt: -1 })
            .skip(skip)
            .limit(limit)
            .select('-rawTextractData'); // Exclude raw data for performance
        const total = await Receipt_1.default.countDocuments({ userId: new mongoose_1.default.Types.ObjectId(userId) });
        res.json({
            success: true,
            receipts,
            pagination: {
 ...
 */
router.post('/restaurants/recalculate', authenticateUser, async (req, res) => {
    try {
        const userId = req.userId;
        console.log('🔄 Starting restaurant totals recalculation...');
        // Get all restaurants for this user
        const restaurants = await Restaurant_1.default.find({ userId: new mongoose_1.default.Types.ObjectId(userId) });
        for (const restaurant of restaurants) {
            let newTotal = 0;
            // Find all completed receipts for this restaurant
            const completedReceipts = await Receipt_1.default.find({
                userId: new mongoose_1.default.Types.ObjectId(userId),
            ...
            for (const receipt of completedReceipts) {
                if (receipt.splitCalculations && receipt.splitCalculations.length > 0) {
                    const meCalculation = receipt.splitCalculations.find(calc => calc.name.toLowerCase() === 'me' || calc.personId.includes('person_me_'));
                    if (meCalculation) {
 ...
 */
router.delete('/restaurants/:name', authenticateUser, async (req, res) => {
    try {
        const userId = req.userId;
        const { name } = req.params;
        const restaurant = await Restaurant_1.default.findOneAndDelete({
            userId: new mongoose_1.default.Types.ObjectId(userId),
 ...
 */
router.delete('/restaurants', authenticateUser, async (req, res) => {
    try {
        const userId = req.userId;
        const result = await Restaurant_1.default.deleteMany({
            userId: new mongoose_1.default.Types.ObjectId(userId)
        });
        res.json({
            success: true,
...
router.get('/:id', authenticateUser, async (req, res) => {
    try {
        ...
        const signedImageUrl = await s3Service_1.S3Service.getSignedUrl(receipt.s3Key, 3600); // 1 hour expiry
        res.json({
            success: true,
            receipt: {
                ...receipt.toObject(),
...
router.put('/:id', authenticateUser, async (req, res) => {
    try {
        ...
        const receipt = await Receipt_1.default.findOneAndUpdate({
            _id: new mongoose_1.default.Types.ObjectId(id),
            userId: new mongoose_1.default.Types.ObjectId(userId)
...
router.delete('/:id', authenticateUser, async (req, res) => {
    try {
        ...
        // Delete from S3
        if (receipt.s3Key) {
            await s3Service_1.S3Service.deleteFile(receipt.s3Key);
        }
        ...
        try {
            ...
            if (restaurant) {
                ...
                if (restaurant.visitCount === 0) {
                    await Restaurant_1.default.findByIdAndDelete(restaurant._id);
                    ...
                    const mostRecentReceipt = await Receipt_1.default.findOne({
                        userId: new mongoose_1.default.Types.ObjectId(userId),
                        restaurantName: receipt.restaurantName,
        ...
        await Receipt_1.default.findByIdAndDelete(id);
        res.json({
            success: true,
 ...
 */
router.post('/:id/people', authenticateUser, async (req, res) => {
    try {
        const userId = req.userId;
        const { id } = req.params;
 ...
 */
router.put('/:id/items/:itemIndex/assign', authenticateUser, async (req, res) => {
    try {
        const userId = req.userId;
        ...
        // Calculate splits
        const splits = splitCalculationService_1.SplitCalculationService.calculateSplit(receipt);
        receipt.splitCalculations = splits;
        receipt.isComplete = splitCalculationService_1.SplitCalculationService.isReceiptComplete(receipt);
        await receipt.save();
        res.json({
            success: true,
 ...
 */
router.put('/:id/distribution', authenticateUser, async (req, res) => {
    try {
        const userId = req.userId;
        const { id } = req.params;
 ...
 */
router.get('/:id/split', authenticateUser, async (req, res) => {
    try {
        const userId = req.userId;
        const { id } = req.params;
        if (!mongoose_1.default.Types.ObjectId.isValid(id)) {
        ...
        const splits = splitCalculationService_1.SplitCalculationService.calculateSplit(receipt);
        const stats = splitCalculationService_1.SplitCalculationService.getReceiptStats(receipt, splits);
        const unassignedItems = splitCalculationService_1.SplitCalculationService.getUnassignedItems(receipt);
        res.json({
            success: true,
            splits,
            stats,
            unassignedItems,
            isComplete: splitCalculationService_1.SplitCalculationService.isReceiptComplete(receipt)
        });
    }
    catch (error) {
 ...
 */
router.put('/:id/finalize-split', authenticateUser, async (req, res) => {
    try {
        const userId = req.userId;
        const { id } = req.params;
...
router.get('/:id/summary', authenticateUser, async (req, res) => {
    try {
        ...
        const splits = splitCalculationService_1.SplitCalculationService.calculateSplit(receipt);
        const summary = splitCalculationService_1.SplitCalculationService.generateShareableSummary(receipt, splits);
        res.json({
            success: true,
            summary,
            splits
 ...
 */
router.put('/restaurants/:name/favorite', authenticateUser, async (req, res) => {
    try {
        const userId = req.userId;
        const { name } = req.params;
        const restaurant = await Restaurant_1.default.findOne({
