...
class TextractService {
     ...
     */
    static async extractReceiptData(s3Bucket, s3Key) {
        try {
     ...
     */
    static async extractReceiptDataFallback(s3Bucket, s3Key) {
        try {
            const params = {
                Document: {
                    S3Object: {
            ...
            const result = await aws_1.textract.analyzeDocument(params).promise();
            if (!result.Blocks) {
     ...
     */
    static parseTextractResponse(blocks) {
     ...
     */
    static containsPrice(text) {
        return /\$?\d+\.?\d*/.test(text);
    }
    /**
     * Check if a line contains a date
     */
    static isDateLine(text) {
     ...
     */
    static async detectTextOnly(s3Bucket, s3Key) {
        try {
            const params = {
                Document: {
                    S3Object: {
            ...
            const result = await aws_1.textract.detectDocumentText(params).promise();
            if (!result.Blocks) {
                throw new Error('No text blocks found in the document');
            }
...
exports.default = TextractService;
